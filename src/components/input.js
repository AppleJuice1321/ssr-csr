...
export default function Input({ label, type, name, placeholder = "", value = "", statusMessage = "" }) {
  ...
  return (
    <label className="block my-6 relative">
      <input 
        className={`py-2 pl-2 rounded-sm w-full peer ${statusMessage ? "outline outline-red-500 outline-2" : null}`}
        type={type} 
        name={name}
        ...
        />
        <span className="absolute top-2 left-2 text-black peer-focus:-top-6 peer-focus:text-sm [transition:all_300ms_ease-in-out] peer-[:not([value=''])]:-top-6 peer-[:not([value=''])]:text-sm peer-[:not([value=''])]:text-white">{label}</span>
